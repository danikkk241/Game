...
function drawSym(item, sym = user_sym){
	...
	if (winner == user_sym) {
		win_text.innerHTML = "Победитель - вы!";
		win_text.style.color = "pink";
		winning.style.backgroundColor = "rgba(255, 20, 147, 0.5)";
	}else if (winner == comp_sym) {
		win_text.innerHTML = "Победитель - компьютер! Попоробуйте еще раз!";
		win_text.style.color = "blue";
		winning.style.backgroundColor = "rgba(0, 0, 255, 0.5)";
	}
	if (winner) {
